...
namespace beb_pos_smc
{

BebPosSmc::BebPosSmc(ros::NodeHandle &nh)
  : nh_(nh),
    nh_priv_("~"),
    ...
    sub_ctrl_enable_(nh_.subscribe("ctrl_enable", 1, &BebPosSmc::CtrlEnableCallback, this)),
    sub_setpoint_pose_(nh_.subscribe("setpoint_pose", 1, &BebPosSmc::SetpointPoseCallback, this)),
    pub_ctrl_cmd_vel_(nh_.advertise<geometry_msgs::Twist>("bebop/cmd_vel", 1)),
    pub_debug_(nh_.advertise<aero_ctrl::Debug>("debug", 1)),
    sub_pose_meas_(nh_.subscribe("slam", 1, &BebPosSmc::SlamCallback, this)),
    sub_filterP_(nh_.subscribe("pose", 1, &BebPosSmc::FilterPCallback, this)),
    sub_filterT_(nh_.subscribe("twist", 1, &BebPosSmc::FilterTCallback, this)),
    sub_filterW_(nh_.subscribe("wrench", 1, &BebPosSmc::FilterWCallback, this)),
    sub_vicon_(nh_.subscribe("vicon", 1, &BebPosSmc::ViconCallback, this)),
    sub_beb_alt_(nh_.subscribe("/bebop/states/ardrone3/PilotingState/AltitudeChanged", 1, &BebPosSmc::BebAltCallback, this)),
    sub_beb_speed_(nh_.subscribe("/bebop/states/ardrone3/PilotingState/SpeedChanged", 1, &BebPosSmc::BebSpeedCallback, this)),
    ...
    ctl_alt_changed_(false)
{
  ...
  util::GetParam(nh_priv_, "smc_c", param_c_, 1.0);
  util::GetParam(nh_priv_, "smc_rho", param_rho_, 0.01);
...
int BebPosSmc::sgn(double v) {
  return (v > 0) - (v < 0);
}

// AMV
void BebPosSmc::CtrlEnableCallback(const std_msgs::Bool& enable_msg)
{
...
// AMV
void BebPosSmc::SetpointPoseCallback(const geometry_msgs::PoseConstPtr& pose_ptr)
{
...
void BebPosSmc::SlamCallback(const geometry_msgs::PoseStampedConstPtr& pose_ptr)
{
  if (!beb_param_recv_) {
...
void BebPosSmc::FilterPCallback(const geometry_msgs::PoseStampedConstPtr& pose_ptr)
{
  msg_debug_.fil_pose = pose_ptr->pose;
}

void BebPosSmc::FilterTCallback(const geometry_msgs::TwistStampedConstPtr& twist_ptr)
{
  msg_debug_.fil_twist = twist_ptr->twist;
}


void BebPosSmc::FilterWCallback(const geometry_msgs::WrenchStampedConstPtr& wrench_ptr)
{
...
void BebPosSmc::ViconCallback(const geometry_msgs::TransformStampedConstPtr& tf_ptr)
{
...
void BebPosSmc::BebAltCallback(const bebop_msgs::Ardrone3PilotingStateAltitudeChangedConstPtr& alt_ptr)
{
  ...
  beb_vz_ = (beb_z_[0] - beb_z_[1])/dt.toSec();

  ros::Duration lag = beb_recv_time_ - alt_ptr->header.stamp;
  
  msg_debug_.beb_alt_lag = lag.toSec();
	msg_debug_.beb_alt = alt_ptr->altitude;
  msg_debug_.beb_vze = beb_vz_;
}


void BebPosSmc::BebSpeedCallback(const bebop_msgs::Ardrone3PilotingStateSpeedChangedConstPtr& speed_ptr)
{
...
bool BebPosSmc::Update()
{
  ...
  if (sensor_fault == 1) {
    double dot_errz = 0.0 - slm_vz_;
    ctrl_twist_.linear.z = param_c_*dot_errz - param_rho_*sgn(dot_errz + param_c_*pos_errors[2]);
    ctrl_twist_.linear.z = CLAMP(ctrl_twist_.linear.z, -0.1, 0.5);
  }
...
void BebPosSmc::Spin()
{
  ...
  while (ros::ok())
  {
    try
    {
      ...
      else if ((ros::Time::now() - pose_recv_time_).toSec() > 0.4) {
        ROS_WARN_THROTTLE(10.0, "[CTL] State feedback is older than 0.4 s! Resetting.");
