...
BebPosPid::BebPosPid(ros::NodeHandle &nh)
  : nh_(nh),
    nh_priv_("~"),
    ...
    sub_ctrl_enable_(nh_.subscribe("ctrl_enable", 1, &BebPosPid::CtrlEnableCallback, this)),
    sub_setpoint_pose_(nh_.subscribe("setpoint_pose", 1, &BebPosPid::SetpointPoseCallback, this)),
    pub_ctrl_cmd_vel_(nh_.advertise<geometry_msgs::Twist>("bebop/cmd_vel", 1)),
    pub_debug_(nh_.advertise<aero_ctrl::Debug>("debug", 1)),
    sub_pose_meas_(nh_.subscribe("slam", 1, &BebPosPid::SlamCallback, this)),
    sub_filterP_(nh_.subscribe("pose", 1, &BebPosPid::FilterPCallback, this)),
    sub_filterT_(nh_.subscribe("twist", 1, &BebPosPid::FilterTCallback, this)),
    sub_filterW_(nh_.subscribe("wrench", 1, &BebPosPid::FilterWCallback, this)),
    sub_vicon_(nh_.subscribe("vicon", 1, &BebPosPid::ViconCallback, this)),
    sub_beb_alt_(nh_.subscribe("/bebop/states/ardrone3/PilotingState/AltitudeChanged", 1, &BebPosPid::BebAltCallback, this)),
    sub_beb_speed_(nh_.subscribe("/bebop/states/ardrone3/PilotingState/SpeedChanged", 1, &BebPosPid::BebSpeedCallback, this)),
    ...
    ctl_alt_changed_(false)
{
  ...
  //subsync_beb_.registerCallback(boost::bind(&BebPosPid::BebSyncCallback, this, _1, _2));
...
// AMV
void BebPosPid::CtrlEnableCallback(const std_msgs::Bool& enable_msg)
{
...
// AMV
void BebPosPid::SetpointPoseCallback(const geometry_msgs::PoseConstPtr& pose_ptr)
{
...
void BebPosPid::SlamCallback(const geometry_msgs::PoseStampedConstPtr& pose_ptr)
{
  if (!beb_param_recv_) {
...
void BebPosPid::FilterPCallback(const geometry_msgs::PoseStampedConstPtr& pose_ptr)
{
  msg_debug_.fil_pose = pose_ptr->pose;
}

void BebPosPid::FilterTCallback(const geometry_msgs::TwistStampedConstPtr& twist_ptr)
{
  msg_debug_.fil_twist = twist_ptr->twist;
}


void BebPosPid::FilterWCallback(const geometry_msgs::WrenchStampedConstPtr& wrench_ptr)
{
...
void BebPosPid::ViconCallback(const geometry_msgs::TransformStampedConstPtr& tf_ptr)
{
...
void BebPosPid::BebAltCallback(const bebop_msgs::Ardrone3PilotingStateAltitudeChangedConstPtr& alt_ptr)
{
...
void BebPosPid::BebSpeedCallback(const bebop_msgs::Ardrone3PilotingStateSpeedChangedConstPtr& speed_ptr)
{
  ...
  while (ros::ok())
  {
    try
    {
      ...
      if (!ctrl_enabled_) {
        ...
        ROS_WARN_THROTTLE(10.0, "[CTL] State feedback is older than 0.5 s! Resetting.");
